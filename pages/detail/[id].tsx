...
const Detail = ({postDetails}:IProps) => {
  ...
  return (
    ...
    <div className='flex w-full absolute left-0 top-0 bg-white flex-wrap lg:flex-nowrap'>
      <div className='relative flex-2 w-[1000px] lg:w-9//12 flex items-center justify-center bg-gray-300'>
        ...
        <div className='relative'>
          <div className='lg:h-[100vh] h-[60vh]'>
            <video ref={videoRef} loop onClick={onVideoClick} className='h-full cursor-pointer' src={post.video.asset.url}></video>
          </div>
          <div className='absolute top-[45%] left-[45%] cursor-pointer'>
             {!playing && (
            <button  className='text-white rounded-full  bg-black' onClick={onVideoClick}>
            ...
            <div>
                <Link href='/' >
                    <div className='flex mt-3 flex-col gap-2'>
                        <p className='flex gap-2 md:text-md text-primary font-bold items-center'>{post.postedBy.userName}{` `}<GoVerified className='text-blue-400 text-md'/></p>
                        <p className='font-medium text-gray-500 hidden md:block text-xs capitalize '>{post.postedBy.userName}</p>
                    </div>
                </Link>
